...
class GitHubApi:
    ...
    def _request(
        ...
        try:
            ...
            raise GitHubApiError(f"GitHub HTTP {exc.code}: {raw[:1000]}") from exc
    ...
    def list_pull_request_files(self, number: int) -> list[dict[str, Any]]:
        ...
        if not isinstance(payload, list):
            raise GitHubApiError("pull request files response must be a list")
    ...
    def merge_pull_request(self, number: int, *, expected_sha: str) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise GitHubApiError("merge response must be an object")
        return payload
    ...
    def get_json_file(self, path: str, *, ref: str = "main") -> tuple[dict[str, Any], str]:
        ...
        if not isinstance(payload, dict):
            raise GitHubApiError(f"{path} response must be an object")
        encoded = payload.get("content")
        sha = payload.get("sha")
        if not isinstance(encoded, str) or not isinstance(sha, str):
            raise GitHubApiError(f"{path} is missing content or sha")
        try:
            raw = base64.b64decode(encoded.replace("\n", ""))
            decoded = json.loads(raw.decode("utf-8"))
        except (ValueError, UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise GitHubApiError(f"{path} is not valid UTF-8 JSON") from exc
        if not isinstance(decoded, dict):
            raise GitHubApiError(f"{path} must contain a JSON object")
        return decoded, sha
